...
from .forms import categoriesmodel, Imageform, Selectcategoriesform, categoriesform
...
def Imagesview(request):
    ...
    if request.method == 'POST' and request.FILES:
        ...
        if form.is_valid():
            ...
            formsave.save()
            user = request.user
            subj = "WELCOME TO MOMENT BY MOMENT PHOTO GALLERY"
            msg = "Thank you for uploading the image . Your image has been successfully uploaded"
            send_mail(subj, msg, settings.EMAIL_HOST_USER, [user.email,])
            messages.success(request, "Your Moment has been Captured Successfully")
            return redirect('/images')
        else:
